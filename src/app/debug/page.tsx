...
export default function UltimateDebugDashboard() {
  const [activeTab, setActiveTab] = useState<'overview' | 'crops' | 'lunar' | 'knf' | 'security' | 'components'>('overview');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#020103] via-[#1a0b2e] to-[#020103] p-8">
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto">
        {[
          { id: 'overview', label: '🏝️ Overview', emoji: '🏝️' },
          { id: 'crops', label: '🌿 Crops & ROI', emoji: '🌿' },
          { id: 'lunar', label: '🌙 Lunar Calendar', emoji: '🌙' },
          { id: 'knf', label: '🧪 KNF Recipes', emoji: '🧪' },
          { id: 'security', label: '🐗 Puaʻa Guard', emoji: '🐗' },
          { id: 'components', label: '⚙️ Components', emoji: '⚙️' }
        ].map(tab => (
          <button
            key={tab.id}
